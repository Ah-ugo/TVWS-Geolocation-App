...
export const QueryTVWS: React.FC = () => {
  ...
  const handleQuery = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(error.message || "Failed to query TVWS data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportToCsv = () => {
    ...
    a.href = url;
    a.download = `tvws-${selectedState}-${selectedLocation}-${
  ...
  const freeChannels =
    result?.channels.filter((ch) => ch.status === "free") || [];
  const occupiedChannels =
    result?.channels.filter((ch) => ch.status === "occupied") || [];
        ...
        <div className="flex items-center space-x-3 mb-6">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-900">
              Query TV White Space
            </h2>
            <p className="text-slate-600">
              Check UHF channel availability for White Space Devices
            </p>
          </div>
        </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-blue-600">
                    {result.totalAvailableBandwidth} MHz
                  </p>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <tbody className="bg-white divide-y divide-slate-200">
                    {result.channels.map((channel) => (
                      <tr
                        key={channel.channel}
                        className={
                          channel.status === "free" ? "bg-green-50" : "bg-white"
                        }
                      >
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              <div className="h-96">
                <MapContainer
                  center={[
                    result.location.coordinates.lat,
                    result.location.coordinates.lon,
                  ]}
                  zoom={13}
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={getChartData()}>
                  ...
                  <Bar
                    ...
                    fill={(entry: any) =>
                      entry.status === "free" ? "#10B981" : "#EF4444"
                    }
                  />
                </BarChart>
              </ResponsiveContainer>
